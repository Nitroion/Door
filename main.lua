...
if getgenv()Door_disable_addons then return end
task.spawn(function()
    ...
    if not supportsFileSystem then
        warn("[Door] Your executor doesn't support the FileSystem API. Addons will not work.")
        return
    end

    if not isfolder("Door/addons") then
        print("[Door] Addons folder doesn't exist. Creating...")
        makefolder("Door/addons")
        return
    end
    
    repeat task.wait() until getgenv().mspaint_loaded == true
    print("[Door] Loading addons...")
    ...
    local function AddAddonElement(LinoriaElement, AddonName, Element)
        if not LinoriaElement then
            warn("[Door] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid Linoria element.")
            return
        end

        if typeof(Element) ~= "table" then
            warn("[Door] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid data.")
            return
        end 

        if typeof(Element.Type) ~= "string" then 
            warn("[Door] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid name.")
            return 
        end

        if typeof(AddonName) ~= "string" then 
            warn("[Door] Element '" .. tostring(Element.Name) .. " (" .. tostring(Element.Type) .. ")' didn't load: Invalid addon name.")
            return 
        end
    ...
    local function createAddonTab(hasAddons: boolean)
        ...
        local addonsText = "This tab is for UN-OFFICIAL addons made for Door. We are not responsible for what addons you will use. You are putting yourself AT RISK since you are executing third-party scripts."
    ...
    for _, file in pairs(listfiles("Door/addons")) do
        print("[mspaint] Loading addon '" .. string.gsub(file, "Door/addons/", "") .. "'...")
        ...
        local success, errorMessage = pcall(function()
            ...
            if typeof(addon.Name) ~= "string" or typeof(addon.Elements) ~= "table" then
                warn("Addon '" .. string.gsub(file, "Door/addons/", "") .. "' didn't load: Invalid Name/Elements.")
                return 
            end

            if typeof(addon.Game) == "string" then
                if addon.Game ~= gameAddonPath and addon.Game ~= "*" then
                    warn("Addon '" .. string.gsub(file, "Door/addons/", "") .. "' didn't load: Wrong game.")
                    return
                end
            elseif typeof(addon.Game) == "table" then
                ...
                warn("Addon '" .. string.gsub(file, "Door/addons/", "") .. "' didn't load: Invalid GameId.")
                return
            end
        ...
        if not success then
            warn("[Door] Failed to load addon '" .. string.gsub(file, "Door/addons/", "") .. "':", errorMessage)
        else
